    ...
    (0 until size).reverse.foreach {i =>
        ...
        taps.find(tap => tap._1 + tap._2 == i) match {
            case Some(tap) => {
                ...
                // xor_poly(poly_count).i2 := io.iEntropy(etp_2_poly)
                poly_count += 1
                // etp_2_poly += 1
    ...
    ElaborationArtefacts.add(
        "ring_generator_base" + ".vivado.xdc",
        {
            ...
            (for(j <- 0 until 8) yield{
              s"""set_property LOC SLICE_X${locHint.loc_x+1}Y${locHint.loc_y-i} [get_cells ${xdcPath}state_${i*8+j}_reg]
            ...
            //   val index = i
              ...
              s"""set_property LOC SLICE_X${locHint.loc_x}Y${locHint.loc_y-i/4} [get_cells $LUT_inst]
            ...
            //   val index = i
              ...
              s"""set_property LOC SLICE_X${locHint.loc_x}Y${locHint.loc_y-i/4-poly_offset} [get_cells $LUT_inst]
                 |set_property DONT_TOUCH true [get_cells $LUT_inst]
                 |set_property BEL ${LUT_name(LUT_index)} [get_cells $LUT_inst]
                 |""".stripMargin
            }.mkString("\n")
